...
Этот документ — **контракт** между редактором и space-manager: точно описывает внутреннюю
...
5. **Неизвестные поля.** Поля YAML, отсутствующие в модели ([02](./02-data-model-formats.md) §1)
   (например, будущие расширения space-manager), при парсинге **отбрасываются** с одноразовым
