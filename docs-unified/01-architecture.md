...
- Аутентификация отсутствует осознанно (вводная №10); сервис не должен запускаться
  с привязкой к публичным сетевым интерфейсам — документировано, дефолт listen без
  host = все интерфейсы Node; **решение**: слушать `127.0.0.1` явно (локальный
  сервис), env `HOST` для переопределения.
